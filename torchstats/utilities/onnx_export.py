...
def export(
    model: torch.nn.Module, input_shapes: List[Any], savepath: Path = None
) -> None:
    """
    ...
    torch.onnx.export(model, tuple(dummy_input), savepath, opset_version=11)
...
def main() -> None:
    ...
    savepath = f"{path}/{args.save if args.save else 'model.onnx'}"
